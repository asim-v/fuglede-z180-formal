...
/-!
# Injectivity of exact evaluation for the K30 coefficient basis, Fix2

This module is independent of the failed first injectivity draft.  It proves
the coefficient formula, the degree bound, exact evaluation of the additive
...
theorem coeffPolynomialV71Fix_coeff_lt_v71Fix2
...
theorem coeffPolynomialV71Fix_coeff_ge_v71Fix2
...
theorem coeffPolynomialV71Fix_coeff_v71Fix2
    (c : Coeff) (n : Nat) :
    (coeffPolynomialV71Fix c).coeff n =
      if n < 12 then c.getD n 0 else 0 := by
  by_cases hn : n < 12
  · rw [if_pos hn]
    exact coeffPolynomialV71Fix_coeff_lt_v71Fix2 c n hn
  · rw [if_neg hn]
    exact coeffPolynomialV71Fix_coeff_ge_v71Fix2 c n
      (Nat.le_of_not_gt hn)

theorem coeffPolynomialV71Fix_degree_lt_v71Fix2 (c : Coeff) :
    (coeffPolynomialV71Fix c).degree < (12 : WithBot Nat) := by
  apply (Polynomial.degree_lt_iff_coeff_zero
    (coeffPolynomialV71Fix c) 12).2
  intro n hn
  exact coeffPolynomialV71Fix_coeff_ge_v71Fix2 c n hn

private theorem list_eq_of_getD_eq_v71Fix2
...
private theorem coeffPolynomialV71Fix_zero_v71Fix2 :
    coeffPolynomialV71Fix coeffZero = 0 := by
  ext n
  by_cases hn : n < 12
  · rw [coeffPolynomialV71Fix_coeff_lt_v71Fix2 coeffZero n hn]
    interval_cases n <;> simp [coeffZero, List.getD]
  · rw [coeffPolynomialV71Fix_coeff_ge_v71Fix2 coeffZero n
      (Nat.le_of_not_gt hn)]
    simp

private theorem coeffPolynomialV71Fix_scalar_v71Fix2 (a : Int) :
    coeffPolynomialV71Fix (scalarCoeff a) = Polynomial.C a := by
  ext n
  by_cases hn : n < 12
  · rw [coeffPolynomialV71Fix_coeff_lt_v71Fix2
      (scalarCoeff a) n hn]
    interval_cases n <;>
      simp [scalarCoeff, List.getD, Polynomial.coeff_C]
  · have hge : 12 ≤ n := Nat.le_of_not_gt hn
    have hn0 : n ≠ 0 := by omega
    rw [coeffPolynomialV71Fix_coeff_ge_v71Fix2
      (scalarCoeff a) n hge]
    rw [Polynomial.coeff_C]
    simp [hn0]

private theorem coeffAdd_getD_of_lt_length_v71Fix2
...
private theorem coeffAdd_getD_of_lt_v71Fix2
    {left right : Coeff}
  ...
  apply coeffAdd_getD_of_lt_length_v71Fix2 left right n
  · simpa [hleftLen] using hn
  · simpa [hrightLen] using hn

private theorem coeffPolynomialV71Fix_add_v71Fix2
    {left right : Coeff}
    (hleft : CoeffWF left) (hright : CoeffWF right) :
  ...
  · simp only [Polynomial.coeff_add]
    rw [coeffPolynomialV71Fix_coeff_lt_v71Fix2
      ...
      coeffPolynomialV71Fix_coeff_lt_v71Fix2 left n hn,
      coeffPolynomialV71Fix_coeff_lt_v71Fix2 right n hn]
    exact coeffAdd_getD_of_lt_v71Fix2 hleft hright n hn
  · have hge : 12 ≤ n := Nat.le_of_not_gt hn
    simp only [Polynomial.coeff_add]
    rw [coeffPolynomialV71Fix_coeff_ge_v71Fix2
        (coeffAdd left right) n hge,
      coeffPolynomialV71Fix_coeff_ge_v71Fix2 left n hge,
      coeffPolynomialV71Fix_coeff_ge_v71Fix2 right n hge]
    simp

@[simp]
theorem evalCoeffV71Fix_coeffZero_v71Fix2 :
    evalCoeffV71Fix coeffZero = 0 := by
  unfold evalCoeffV71Fix
  rw [coeffPolynomialV71Fix_zero_v71Fix2]
  simp

@[simp]
theorem evalCoeffV71Fix_scalarCoeff_v71Fix2 (a : Int) :
    evalCoeffV71Fix (scalarCoeff a) = (a : Complex) := by
  unfold evalCoeffV71Fix
  rw [coeffPolynomialV71Fix_scalar_v71Fix2]
  simp

theorem evalCoeffV71Fix_coeffAdd_v71Fix2
    {left right : Coeff}
    (hleft : CoeffWF left) (hright : CoeffWF right) :
    evalCoeffV71Fix (coeffAdd left right) =
      evalCoeffV71Fix left + evalCoeffV71Fix right := by
  unfold evalCoeffV71Fix
  rw [coeffPolynomialV71Fix_add_v71Fix2 hleft hright]
  simp

theorem evalCoeffV71Fix_injective_of_wf_v71Fix2
    {left right : Coeff}
    (hleft : CoeffWF left) (hright : CoeffWF right)
  ...
  have hdegree : P.degree < (12 : WithBot Nat) := by
    ...
    exact (Polynomial.degree_sub_le _ _).trans_lt
      (max_lt
        (coeffPolynomialV71Fix_degree_lt_v71Fix2 left)
        (coeffPolynomialV71Fix_degree_lt_v71Fix2 right))
  ...
  have hrightLen : right.length = 12 := by
    simpa [CoeffWF] using hright
  apply list_eq_of_getD_eq_v71Fix2 (0 : Int)
  ...
  · intro n hnleft
    ...
    rw [coeffPolynomialV71Fix_coeff_lt_v71Fix2 left n hn,
      coeffPolynomialV71Fix_coeff_lt_v71Fix2 right n hn] at hcoeff
    exact hcoeff

#print axioms coeffPolynomialV71Fix_coeff_v71Fix2
#print axioms coeffPolynomialV71Fix_degree_lt_v71Fix2
#print axioms evalCoeffV71Fix_coeffAdd_v71Fix2
#print axioms evalCoeffV71Fix_injective_of_wf_v71Fix2
