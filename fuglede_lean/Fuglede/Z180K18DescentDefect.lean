...
/-!
# A failed cardinality-eighteen descent produces a five-factor defect

The order of a frequency in `ZMod 180` is either the order of its image in
`ZMod 36`, or five times that order.  The generic projected-mask and order
lemmas already used by the cardinality-six descent therefore apply unchanged
to the cardinality-eighteen projection.
-/
...
eighteen. -/
def Z180K18Projection36FiveFactorDefect
...
theorem z180_k18_fiveFactorDefect_of_not_zero_descent
    {A L : Finset (ZMod 180)}
    (hSpec : CyclotomicSpectrum 180 A L) (hcard : A.card = 18)
    (hnotdesc : ¬ Z180K18Projection36ZeroDescends A L) :
    Z180K18Projection36FiveFactorDefect A L := by
  classical
  simp only [Z180K18Projection36ZeroDescends, not_forall,
    _root_.not_imp] at hnotdesc
  obtain ⟨l₁, hl₁, l₂, hl₂, hne, hnotzero⟩ := hnotdesc
  let d : ZMod 180 := l₁ - l₂
  let d36 : ZMod 36 := z180K18Projection36 l₁ - z180K18Projection36 l₂
  ...
  have hinjA18 := z180_k18_projection36_injOn hSpec hcard
  have hinjA6 : Set.InjOn z180K6Projection36 (A : Set (ZMod 180)) := by
    simpa [z180K18Projection36, z180K6Projection36] using hinjA18
  have hnotlow : ¬ cyclotomic m Int ∣ maskPolynomial 180 A := by
    intro hlow
    apply hnotzero
    unfold CyclotomicZero
    change cyclotomic m Int ∣
      maskPolynomial 36 (A.image z180K18Projection36)
    rw [show A.image z180K18Projection36 =
        A.image z180K6Projection36 by
          simp [z180K18Projection36, z180K6Projection36]]
  ...
  have hmap : z180K6Projection36 d = d36 := by
    simp [d, d36, z180K18Projection36, z180K6Projection36, map_sub]
...
exact cardinality-eighteen theorem in `ZMod 36`. -/
theorem z180_k18_fiveFactorDefect_of_non_tiler
    {A L : Finset (ZMod 180)}
    (hSpec : CyclotomicSpectrum 180 A L) (hcard : A.card = 18)
    (hInduction : Z36SpectralCard18ImpliesTile)
    (hnot : ¬ ∃ B : Finset (ZMod 180), Tiles A B) :
    Z180K18Projection36FiveFactorDefect A L := by
  apply z180_k18_fiveFactorDefect_of_not_zero_descent hSpec hcard
  exact z180_k18_not_zero_descent_of_non_tiler
    hSpec hcard hInduction hnot

#print axioms z180_k18_fiveFactorDefect_of_not_zero_descent
#print axioms z180_k18_fiveFactorDefect_of_non_tiler
