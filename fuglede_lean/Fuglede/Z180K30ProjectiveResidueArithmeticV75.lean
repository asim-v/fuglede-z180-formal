...
def z36RawPointV75 (x : ZMod 36) : RawPoint :=
  ⟨x.val, ZMod.val_lt x⟩

@[simp]
theorem z36RawPointV75_val (x : ZMod 36) :
    (z36RawPointV75 x).val = x.val := rfl

/-- A six-tuple of residues, viewed in the raw-certificate list format. -/
def z36RawSetV75 (X : Fin 6 → ZMod 36) : RawSet :=
  List.ofFn fun i => z36RawPointV75 (X i)

@[simp]
theorem z36RawPointV75_natCast_rawPoint (x : RawPoint) :
    z36RawPointV75 (x.val : ZMod 36) = x := by
  apply Fin.ext
  simp [z36RawPointV75, ZMod.val_natCast, Nat.mod_eq_of_lt x.isLt]

@[simp]
theorem z36RawSetV75_natCast_rawPoints (X : Fin 6 → RawPoint) :
    z36RawSetV75 (fun i => (X i).val : Fin 6 → ZMod 36) =
      List.ofFn X := by
  simp [z36RawSetV75]
...
theorem z36_dvd_mul_natAbsDiff_of_mul_sub_eq_zero_v75
    (k x y : ZMod 36)
    (hzero :
      (k.val : ZMod 36) *
...
`differenceGcd`. -/
private theorem thirtySix_dvd_mul_foldl_point_gcd_v75
...
theorem thirtySix_dvd_mul_differenceGcd_cons_v75
    (k : Nat) (head : RawPoint) (tail : List RawPoint)
    (htail : ∀ point ∈ tail,
      36 ∣ k * natAbsDiff point.val head.val) :
    36 ∣ k * differenceGcd (head :: tail) := by
  rw [differenceGcd]
  apply thirtySix_dvd_mul_foldl_point_gcd_v75
  · exact ⟨k, by omega⟩
  · exact htail

/-- Every `differenceGcd` is a divisor of 36, since its fold starts at 36. -/
private theorem foldl_point_gcd_dvd_initial_v75
...
theorem z36_val_mod_div_differenceGcd_eq_of_mul_sub_zero_v75
    (x y : ZMod 36) (V : Fin 6 → ZMod 36)
    (hzero : ∀ l, (x - y) * (V l - V 0) = 0) :
    x.val % (36 / differenceGcd (z36RawSetV75 V)) =
      y.val % (36 / differenceGcd (z36RawSetV75 V)) := by
  let g := differenceGcd (z36RawSetV75 V)
  have hfold : 36 ∣ (x - y).val * g := by
    dsimp only [g]
    rw [z36RawSetV75, List.ofFn_succ]
    apply thirtySix_dvd_mul_differenceGcd_cons_v75
    ...
    simpa only [z36RawPointV75_val] using
      z36_dvd_mul_natAbsDiff_of_mul_sub_eq_zero_v75
        (x - y) (V i.succ) (V 0) hzmod
  have hg : g ∣ 36 := differenceGcd_dvd_thirtySix_v75 _
  have hgpos : 0 < g := by
  ...
  have hquot : 36 / g ∣ (x - y).val :=
    thirtySix_div_g_dvd_of_dvd_mul_v75 g (x - y).val hg hgpos hfold
  have hquot36 : 36 / g ∣ 36 := by
    exact ⟨g, (Nat.div_mul_cancel hg).symm⟩
  exact z36_val_mod_eq_of_dvd_sub_val_v75
...
theorem classCount_z36RawSetV75_le_two_of_pair_v75
    (X : Fin 6 → ZMod 36) (modulus : Nat) (p q : Fin 6)
    (hpair : ∀ i,
      (X i).val % modulus = (X p).val % modulus ∨
        (X i).val % modulus = (X q).val % modulus) :
    classCount (z36RawSetV75 X) modulus ≤ 2 := by
  have h0 := hpair (0 : Fin 6)
  have h1 := hpair (1 : Fin 6)
  have h2 := hpair (2 : Fin 6)
  have h3 := hpair (3 : Fin 6)
  have h4 := hpair (4 : Fin 6)
  have h5 := hpair (5 : Fin 6)
  by_cases hpq :
      (X p).val % modulus = (X q).val % modulus
  · rcases h0 with h0 | h0 <;>
      ...
      simp [classCount, z36RawSetV75, h0, h1, h2, h3, h4, h5, hpq]
  · rcases h0 with h0 | h0 <;>
      rcases h1 with h1 | h1 <;>
      rcases h2 with h2 | h2 <;>
      rcases h3 with h3 | h3 <;>
...
theorem z36FourierBlockV74Fix_rows_classCount_le_two_v75
    (U V : Fin 6 → ZMod 36) (p q : Fin 6)
    (hrows : Z180K30RowsCoveredByTwoProjectiveV73Fix
      (z36FourierBlockV74Fix U V) p q) :
    classCount (z36RawSetV75 U)
      (36 / differenceGcd (z36RawSetV75 V)) ≤ 2 := by
  apply classCount_z36RawSetV75_le_two_of_pair_v75 U _ p q
  intro i
  rcases hrows i with ⟨scale, hrow⟩ | ⟨scale, hrow⟩
  · left
    exact z36_val_mod_div_differenceGcd_eq_of_mul_sub_zero_v75
      (U i) (U p) V
...
`classCount ≤ 2` alternative. -/
theorem z36FourierBlockV74Fix_columns_classCount_le_two_v75
    (U V : Fin 6 → ZMod 36) (p q : Fin 6)
    (hcolumns : Z180K30ColumnsCoveredByTwoProjectiveV73Fix
      (z36FourierBlockV74Fix U V) p q) :
    classCount (z36RawSetV75 V)
      (36 / differenceGcd (z36RawSetV75 U)) ≤ 2 := by
  apply classCount_z36RawSetV75_le_two_of_pair_v75 V _ p q
  intro l
  rcases hcolumns l with ⟨scale, hcolumn⟩ | ⟨scale, hcolumn⟩
  · left
    apply z36_val_mod_div_differenceGcd_eq_of_mul_sub_zero_v75
      (V l) (V p) U
    intro a
...
theorem z36FourierBlockV74Fix_classCount_dichotomy_v75
    (U V : Fin 6 → ZMod 36) (p q : Fin 6)
    (hprojective :
    ...
    classCount (z36RawSetV75 U)
          (36 / differenceGcd (z36RawSetV75 V)) ≤ 2 ∨
      classCount (z36RawSetV75 V)
          (36 / differenceGcd (z36RawSetV75 U)) ≤ 2 := by
  rcases hprojective with hrows | ⟨c, d, hcolumns⟩
  · exact Or.inl
      (z36FourierBlockV74Fix_rows_classCount_le_two_v75
        U V p q hrows)
  · exact Or.inr
      (z36FourierBlockV74Fix_columns_classCount_le_two_v75
        U V c d hcolumns)

/-- Boolean endpoint consumed by `exceptionalPairB`. -/
theorem z36FourierBlockV74Fix_rankTwoProjectiveCoverB_v75
    (U V : Fin 6 → ZMod 36) (p q : Fin 6)
    (hprojective :
      Z180K30RowsCoveredByTwoProjectiveV73Fix
          (z36FourierBlockV74Fix U V) p q ∨
        ∃ c d : Fin 6,
          Z180K30ColumnsCoveredByTwoProjectiveV73Fix
            (z36FourierBlockV74Fix U V) c d) :
    rankTwoProjectiveCoverB (z36RawSetV75 U) (z36RawSetV75 V) = true := by
  rcases z36FourierBlockV74Fix_classCount_dichotomy_v75
...
theorem z36FourierBlockV74Fix_rankTwoProjectiveCoverB_of_span_two_v75
    (U V : Fin 6 → ZMod 36)
    ...
    rankTwoProjectiveCoverB (z36RawSetV75 U) (z36RawSetV75 V) = true := by
  apply z36FourierBlockV74Fix_rankTwoProjectiveCoverB_v75 U V p q
  exact z36FourierBlockV74Fix_projective_dichotomy_of_span_two
    U V p q alpha beta hspan

#print axioms z36_dvd_mul_natAbsDiff_of_mul_sub_eq_zero_v75
#print axioms z36_val_mod_div_differenceGcd_eq_of_mul_sub_zero_v75
#print axioms z36FourierBlockV74Fix_classCount_dichotomy_v75
#print axioms z36FourierBlockV74Fix_rankTwoProjectiveCoverB_of_span_two_v75
