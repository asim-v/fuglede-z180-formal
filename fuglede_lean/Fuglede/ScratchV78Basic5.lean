...
theorem rawDifference_cast_scratch5 (left right : RawPoint) :
...
private theorem complex_sum_map_flatMap_scratch5
...
theorem evalCoeff_coeffSum_zeta_scratch5
    (exponents : List Nat) :
...
theorem evalCoeff_energy_scratch5
  ...
  unfold energy
  rw [evalCoeffV71Fix_coeffSum_v76Fix]
  · rw [complex_sum_map_flatMap_scratch5]
    apply congrArg List.sum
    apply List.map_congr_left
    intro row _hrow
    change
      (V.map fun column : RawPoint =>
        evalCoeffV71Fix
          (zetaPower
            (difference * ((row.val + 36 - column.val) % 36)))).sum =
      (V.map fun column : RawPoint =>
        ZMod.stdAddChar
          ((difference : ZMod 36) *
            (((row.val + 36 - column.val) % 36 : Nat) : ZMod 36))).sum
    apply congrArg List.sum
    apply List.map_congr_left
...
theorem evalCoeff_trace_scratch5
    (U V : RawSet) :
  ...
  · rw [complex_sum_map_flatMap_scratch5]
    ...
    change
      (U.map fun right : RawPoint =>
        evalCoeffV71Fix
          (energy V ((left.val + 36 - right.val) % 36))).sum = _
    apply congrArg List.sum
    apply List.map_congr_left
    intro right _hright
    exact evalCoeff_energy_scratch5 V
