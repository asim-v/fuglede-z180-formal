...
theorem quadratic_third_root_is_one_of_two_v72
    {A B C x y z : Complex}
    (hA : A ≠ 0) (hxy : x ≠ y)
    (hx : A * x ^ 2 + B * x + C = 0)
    (hy : A * y ^ 2 + B * y + C = 0)
  ...
  · right
    ...
    exact sub_eq_zero.mp ((mul_eq_zero.mp hyz).resolve_left hA)

theorem unit_affine_phase_has_two_values_v72
  ...
  have hquadratic (j : Fin 6) :
    ...
    rw [hzAffine j] at hunitZ
    simp only [map_add, map_mul] at hunitZ
    calc
      (star a * b) * (t j) ^ 2 +
  ...
  · push_neg at hall
    ...
    refine ⟨0, q, ?_⟩
    intro j
    exact quadratic_third_root_is_one_of_two_v72
      hleading hq.symm (hquadratic 0) (hquadratic q) (hquadratic j)

#print axioms quadratic_third_root_is_one_of_two_v72
#print axioms unit_affine_phase_has_two_values_v72
