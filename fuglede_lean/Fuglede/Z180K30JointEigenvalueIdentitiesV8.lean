...
structure Z180K30GramOperatorDataV8 where
  core : Z180K30PositiveCommutingGramFamilyV7
  sum_apply :
    forall x, (sum fun r => core.family.gram r x) =
      (30 : Complex) • x
...
theorem Z180K30GramOperatorDataV8.sum_eigenvalue_eq_thirty
    (data : Z180K30GramOperatorDataV8) (k : Fin 6) :
    (sum fun r => data.core.eigenvalue k r) = 30 := by
  ...
  have hcomplex :
      (sum fun r => (data.core.eigenvalue k r : Complex)) = 30 := by
    simpa using hinner
  exact_mod_cast hcomplex

theorem Z180K30GramOperatorDataV8.common_cross_product
    (data : Z180K30GramOperatorDataV8) (k : Fin 6) :
    exists q : Real, forall r s, Not (r = s) ->
      data.core.eigenvalue k r * data.core.eigenvalue k s = q := by
  obtain •Q, hQ• := data.commonProduct
  refine •Complex.re
...
theorem Z180K30GramOperatorDataV8.sum_eigenvalue_eq_thirtySix
    (data : Z180K30GramOperatorDataV8) (r : Fin 5) :
    (sum fun k => data.core.eigenvalue k r) = 36 := by
  have ht := data.trace_eq r
  rw [LinearMap.trace_eq_matrix_trace Complex
    data.core.family.jointBasis.toBasis] at ht
  have hcomplex :
      (sum fun k => (data.core.eigenvalue k r : Complex)) = 36 := by
    simpa [Matrix.trace, LinearMap.toMatrix_apply,
      data.core.gram_apply_jointBasis] using ht
  exact_mod_cast hcomplex

noncomputable def Z180K30GramOperatorDataV8.toJointEigenvalueData
    (data : Z180K30GramOperatorDataV8) :
...
theorem Z180K30GramOperatorDataV8.dichotomy
    (data : Z180K30GramOperatorDataV8) :
...
#print axioms Z180K30GramOperatorDataV8.dichotomy
