...
/-!
# Semantic closure of the `k = 18`, `m = 12`, `q = 45` frontier

This module replaces the canonical 54-pattern table by a structural
certificate.  `Phi_45` and the forced `Phi_9` zero make every fifth level
individually `Phi_9`-zero.  Positivity and total cardinality give one level
of size six and four of size three.  The `Phi_60` defect makes all projected
level masks congruent modulo `Phi_12`; the arithmetic module proves that the
three possible singleton signatures are pairwise disjoint.  Hence all four
singleton levels would have one residue type, contributing mass at least
four to a modulo-nine fibre whose global mass is exactly two.

No finite table, native evaluation, or uninhabited interface is used.
-/
...
/-- The five level masses partition the global projected mass in each
modulo-nine fibre. -/
theorem z180_k18_m12_sum_level_nineFiberMass
...
/-- Structural exclusion of `Phi_45` in the base-order-twelve defect
branch.  The spectral-pair hypothesis is not needed after the defect and
forced core factors have been extracted. -/
theorem z180_k18_m12_not_phi45_structural
    {A L : Finset (ZMod 180)}
    (hcard : A.card = 18)
  ...
  have h60 : cyclotomic 60 Int ∣ maskPolynomial 180 A := by
    simpa using hhigh
  have hnot12 : ¬ cyclotomic 12 Int ∣ maskPolynomial 180 A := by
    simpa using hnotlow
  ...
  have hpos : ∀ c : ZMod 5, 0 < z180FifthFiberCard A c :=
    z180_k18_fifthFiberCard_pos_of_fiveFactorDefect
      ...
      z180K18M12OrderTwelveBaseFrequency_order (by simpa using h60) hnot12
  ...
  have hprofile : ∀ c : ZMod 5,
      ...
      ∃ t : Fin 3, Z180K18M12Phi45SingletonProfile
        (z180K18M12LevelIndicator A c) t := by
    intro c hc
    apply z180_k18_m12_phi45_singletonProfile_of_phi9
    · intro i
  ...
  have hmassCanonical : ∀ r : ZMod 9,
      z180K18M12NineFiberMass (z180K18M12LevelIndicator A 0) r +
        ...
        z180K18M12NineFiberMass (z180K12ProjectionIndicator A) r := by
    intro r
    have h := z180_k18_m12_sum_level_nineFiberMass A r
  ...
  have himpossible
    ...
    exact z180_k18_m12_phi45_four_singletons_impossible
...
/-- Inhabitant of the last `m = 12` residual envelope interface. -/
theorem z180_k18_m12_phi45_envelope :
    Z180K18M12Phi45EnvelopeInterface := by
  intro A L hSpec hcard hcore hdefect
  exact z180_k18_m12_not_phi45_structural hcard hcore hdefect

#print axioms z180_k18_m12_sum_level_nineFiberMass
#print axioms z180_k18_m12_not_phi45_structural
#print axioms z180_k18_m12_phi45_envelope
