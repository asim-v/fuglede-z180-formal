...
/-!
# Complex reflection of the raw K30 Gram certificate (V76 Fix2)
-/
...
abbrev RawComplexGramV76Fix2 := Matrix (Fin 6) (Fin 6) Complex

def rawGramFlatIndexV76Fix2 (row column : Fin 6) : Nat :=
  row.val * 6 + column.val

noncomputable def evalGramV76Fix2 (entries : Gram) : RawComplexGramV76Fix2 :=
  fun row column =>
    evalCoeffV71Fix
      (entries.getD (rawGramFlatIndexV76Fix2 row column) coeffZero)

@[simp]
theorem gramTarget30_length_v76Fix2 : gramTarget30.length = 36 := by
  simp [gramTarget30]

theorem gramTarget30_getD_wf_v76Fix2 (row column : Fin 6) :
    CoeffWF
      (gramTarget30.getD
        (rawGramFlatIndexV76Fix2 row column) coeffZero) := by
  fin_cases row <;> fin_cases column <;>
    simp [CoeffWF, rawGramFlatIndexV76Fix2, gramTarget30,
      coeffZero, scalarCoeff]

theorem evalGramV76Fix2_gramTarget30 :
    evalGramV76Fix2 gramTarget30 =
      (30 : Complex) • (1 : RawComplexGramV76Fix2) := by
  ext row column
  fin_cases row <;> fin_cases column <;>
    norm_num [evalGramV76Fix2, rawGramFlatIndexV76Fix2, gramTarget30,
      evalCoeffV71Fix_coeffZero_v71Fix3,
      evalCoeffV71Fix_scalarCoeff_v71Fix3]

structure RawFiveGramComplexBridgeV76Fix2
    (U : Fin 5 → RawSet) (V : RawSet) where
  traceSquare_eval : ∀ r : Fin 5,
    evalCoeffV71Fix (gramTraceSquare (U r) V) =
      Matrix.trace
        (evalGramV76Fix2 (gram (U r) V) *
          evalGramV76Fix2 (gram (U r) V))
  gramSum_eval :
    evalGramV76Fix2
        (gramSum (List.ofFn fun r : Fin 5 => gram (U r) V)) =
      ∑ r : Fin 5, evalGramV76Fix2 (gram (U r) V)
  gramSum_length :
  ...
  gramSum_entry_wf : ∀ row column : Fin 6,
    CoeffWF
      ((gramSum (List.ofFn fun r : Fin 5 => gram (U r) V)).getD
        (rawGramFlatIndexV76Fix2 row column) coeffZero)

private theorem list_eq_of_getD_eq_v76Fix2
...
theorem fiveGramStarB_of_complex_certificate_v76Fix2
    (U : Fin 5 → RawSet) (V : RawSet)
    (bridge : RawFiveGramComplexBridgeV76Fix2 U V)
    ...
    (htrace : ∀ r : Fin 5,
      Matrix.trace
        (evalGramV76Fix2 (gram (U r) V) *
          evalGramV76Fix2 (gram (U r) V)) = (936 : Complex))
    (hsum :
      (∑ r : Fin 5, evalGramV76Fix2 (gram (U r) V)) =
        (30 : Complex) • (1 : RawComplexGramV76Fix2)) :
  ...
  have hsumEvaluated :
      evalGramV76Fix2
          (gramSum (List.ofFn fun r : Fin 5 => gram (U r) V)) =
        evalGramV76Fix2 gramTarget30 := by
    calc
      evalGramV76Fix2
          (gramSum (List.ofFn fun r : Fin 5 => gram (U r) V)) =
          ∑ r : Fin 5, evalGramV76Fix2 (gram (U r) V) :=
        bridge.gramSum_eval
      _ = (30 : Complex) • (1 : RawComplexGramV76Fix2) := hsum
      _ = evalGramV76Fix2 gramTarget30 :=
        evalGramV76Fix2_gramTarget30.symm
  have hsumRaw :
      gramSum (List.ofFn fun r : Fin 5 => gram (U r) V) =
        gramTarget30 := by
    apply list_eq_of_getD_eq_v76Fix2 coeffZero
    · exact bridge.gramSum_length.trans gramTarget30_length_v76Fix2.symm
    · intro n hn
      have hn36 : n < 36 := by
      ...
      have hindex : rawGramFlatIndexV76Fix2 row column = n := by
        dsimp [rawGramFlatIndexV76Fix2, row, column]
        exact Nat.div_add_mod n 6
      apply evalCoeffV71Fix_injective_of_wf_v71Fix3
        (by simpa [hindex] using bridge.gramSum_entry_wf row column)
        (by simpa [hindex] using
          gramTarget30_getD_wf_v76Fix2 row column)
      have hentry := congrFun (congrFun hsumEvaluated row) column
      simpa [evalGramV76Fix2, hindex] using hentry
...
#print axioms evalGramV76Fix2_gramTarget30
#print axioms fiveGramStarB_of_complex_certificate_v76Fix2
