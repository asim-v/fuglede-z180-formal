...
theorem rawDifference_cast_scratch7 (left right : RawPoint) :
...
private theorem complex_sum_map_flatMap_scratch7
...
theorem evalCoeff_coeffSum_zeta_scratch7
    (exponents : List Nat) :
...
theorem evalCoeff_energy_scratch7
  ...
  unfold energy
  rw [evalCoeffV71Fix_coeffSum_v76Fix]
  · rw [complex_sum_map_flatMap_scratch7]
    apply congrArg List.sum
    apply List.map_congr_left
    intro row _hrow
    rw [List.map_map]
...
theorem evalCoeff_trace_scratch7
    (U V : RawSet) :
  ...
  · rw [complex_sum_map_flatMap_scratch7]
    ...
    apply List.map_congr_left
    intro right _hright
    exact evalCoeff_energy_scratch7 V
