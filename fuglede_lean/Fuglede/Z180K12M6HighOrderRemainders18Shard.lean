...
/-!
# Characteristic-five projected remainders for order eighteen

This shard contains the bounded integral argument used by the order-ninety
...
private theorem remainderCoeff_eighteen_high (b : Fin 36 → Int) :
  ...
  have hdiv : cyclotomic 18 Int ∣ projection36IndicatorMask r :=
    cyclotomic_dvd_projection36RemainderRemoved 18 b (by
...
private theorem signed_four_four_five_mul_eq_zero_eighteen
...
/-- A mapped `Phi_18` divisor forces its six bounded integral remainders to
vanish. -/
theorem projection36_phi18_remainders_zero_of_mod_five_dvd_high
  ...
  rcases remainderCoeff_eighteen_high b with ⟨h0, h1, h2, h3, h4, h5⟩
  ...
  rw [h4] at hk4
  rw [h5] at hk5
  dsimp only [projection36Phi18R0] at hk0
  dsimp only [projection36Phi18R1] at hk1
  dsimp only [projection36Phi18R2] at hk2
  dsimp only [projection36Phi18R3] at hk3
  dsimp only [projection36Phi18R4] at hk4
  dsimp only [projection36Phi18R5] at hk5
  dsimp only [projection36Phi18R0, projection36Phi18R1,
    projection36Phi18R2, projection36Phi18R3,
    projection36Phi18R4, projection36Phi18R5]
  constructor
  · exact signed_four_four_five_mul_eq_zero_eighteen
  ...
  constructor
  · have h := signed_four_four_five_mul_eq_zero_eighteen
...
/-- Zero explicit remainders recover integral `Phi_18` divisibility. -/
theorem cyclotomic_eighteen_dvd_projection36Indicator_of_remainders_zero_high
    (b : Fin 36 → Int)
  ...
  intro j hj
  rw [show Nat.totient 18 = 6 by decide] at hj
  interval_cases j
  · exact h0.trans z0
  · exact h1.trans z1
  · exact h2.trans z2
...
#print axioms projection36_phi18_remainders_zero_of_mod_five_dvd_high
#print axioms cyclotomic_eighteen_dvd_projection36Indicator_of_remainders_zero_high
