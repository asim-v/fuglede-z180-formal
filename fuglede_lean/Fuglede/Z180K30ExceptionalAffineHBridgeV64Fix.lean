...
This repaired V64 bridge uses only the core Boolean eliminators exposed by
the current Lean toolchain.
-/

namespace Fuglede.Z180K30ExceptionalRawV2

theorem HCosetB_of_valid_affine_H_v64Fix
    (V : RawSet)
  ...
  have hImageAll :
      (affineSet unit translation V).all
        (fun point => decide (point ∈ z180K30ExceptionalH)) = true := by
    unfold rawSetEqB at hsetEq
    cases hlen :
        ((affineSet unit translation V).length ==
          z180K30ExceptionalH.length)
    · simp [hlen] at hsetEq
    · simpa [hlen] using hsetEq
...
#print axioms HCosetB_of_valid_affine_H_v64Fix
